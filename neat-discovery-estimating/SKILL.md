---
name: neat-discovery-estimating
description: Estimate scoped MVP requirements from Phase 2 analysis - produces T-shirt size estimates (XS-XXL) for MVP core vs deferred features, accounting for technical complexity and risk
---

# Discovery Estimating

**Role:** Solutions architect sizing scoped requirements for effort and
feasibility.

## Overview

Provide T-shirt size estimates (XS-XXL) for MVP-scoped requirements from
Phase 2 analysis, accounting for technical complexity and risk. Make
assumptions explicit, express estimates as ranges.
...
- Inform executive decision-making on phasing
...
### Step 3: Check Existing Estimates

Check if estimation file exists:

```bash
ls docs/{project-name}/03-estimating/mvp-estimates.md 2>/dev/null
```
...
```markdown
mvp-estimates.md:
# Estimation: {Project Name} - MVP Scope
...
mkdir -p docs/{project-name}/03-estimating

# Write estimation file
# (Use Write tool for mvp-estimates.md with REQ-IDs in story titles)

# Update traceability matrix
# (Use Edit tool to add columns to docs/{project-name}/traceability-matrix.md)
# Add: Est Story + Estimate + Dependencies columns
...
- docs/{project-name}/traceability-matrix.md (updated with estimates and dependencies)
...
Decision Options:
1. ✅ PROCEED: Scope and effort are acceptable → Run /neat-discovery-designing
2. 🔄 RE-SCOPE: MVP effort too large → Re-run /neat-discovery-analysing with tighter scope
3. ⏸️  DEFER: Effort exceeds capacity → Pause discovery, revisit later

Recommendation: If MVP estimate > 9 months, consider re-scoping Phase 2 before designing architecture.
...
- **Purpose:** Size the scoped requirements for executive decision-making and phasing

## File Format

**Location:** `docs/{project-name}/03-estimating/mvp-estimates.md`
